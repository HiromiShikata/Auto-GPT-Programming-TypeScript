...
exports.FixErrorUseCase = void 0;
class FixErrorUseCase {
    constructor(fileRepository, bashExecutor, patchCreator, dateTimeRepository) {
        this.fileRepository = fileRepository;
        this.bashExecutor = bashExecutor;
        this.patchCreator = patchCreator;
        this.dateTimeRepository = dateTimeRepository;
        this.run = async (context, projectRootPath, testCommand, commandToSeeProjectStructure) => {
            let relatedSourceFiles = [];
            ...
            for (let i = 0; i < 60; i++) {
                ...
                const { stderr, stdout, exitStatusCode } = await this.bashExecutor.execute(`cd ${projectRootPath} && ${commandToSeeProjectStructure} && ${testCommand}`);
                if (exitStatusCode === 0) {
...
${stderr
                    .split('\n')
                    .filter((m) => !m.includes('Debugger listening on ws://') &&
...
`;
                ...
                const res = await this.patchCreator.fix(context, relatedSourceFiles, errorMessage);
...
`);
        ...
        EOF
        `;
                    const patchApplyResult = await this.bashExecutor.execute(patchApplyCommand);
...
${JSON.stringify(patchApplyResult)}`);
                ...
                relatedSourceFiles = await this.fileRepository.read(res.filePathsToRead, projectRootPath);
            }
            throw new Error('Failed to fix error');
        };
        this.createBranchName = (context) => {
...
exports.FixErrorUseCase = FixErrorUseCase;
//# sourceMappingURL=FixErrorUseCase.js.map