...
describe('FixErrorHandler', () => {
  test('iemeci / uber', async () => {
    const fixErrorHandler = await fixErrorCliHandler(
      `Please fix the error`,
      '/home/hiromi/git/iemeci/iemeci-order-service-third/uber-service',
      'npx eslint src/usecases/*.ts --fix',
      `tree -I 'adapters|__tests__|node_modules|dist|reports|*.orig|*.rej|bin|tmp|migrations|logs|generated|*.mp4'`,
      'gpt-4',
    );
